...
function SetMyInfo() {
  ...
  const { data, mutate, isSuccess, error } = SetData(
    SetUserInfo,
    "SetUserData"
  );
  const {
    data: EditAddressData,
    mutate: EditAdressMutate,
    isSuccess: EditAddressSuccess,
    error: EditAddressError,
  } = SetData(EditAddress, "EditAddress");
  ...
  const HandleSubmit = (data) => {
    if (edit) {
      EditAdressMutate({
        ...
        address: address,
        chooseAddress:
          getValues().shipaddress && getValues().billingaddress
            ? "both"
            : getValues().shipaddress
            ? "shipping"
  ...
  useEffect(() => {
    ...
    if (data?.data?.status == "true_shipping") {
      ...
      localStorage.setItem(`${UserId}addressId`, data.data.addressId);
       ...
       console.log('aedeadadaeijdaeijdiej');
    ...
    <div className="Set_my_info_container">
      ...
      <div className="Set_my_info">
        <form action="" onSubmit={handleSubmit(HandleSubmit)} className="Form">
          ...
          <div
            className="Checkbox"
            style={edit ? { visibility: "hidden" } : { visibility: "visible" }}
          >
            <div className="Set_shiping_box">
              <input {...register("shipaddress")} type="checkbox" />
              <span>Set as default shipping address</span>
            </div>
            <div className="Set_billing_box">
              <input {...register("billingaddress")} type="checkbox" />
